 ...
 *   - **liquidity**— funded into a `TesseraAMM` pool, which lifts the redeemable
 *                    value of every LP share in it, the app's own included
...
contract TesseraFeeCollector {
    ...
    /**
     * @notice Where the liquidity leg goes when no AMM is configured.
     *
     * Kept as a slot so the leg has somewhere to land on a deployment whose AMM
     * is not wired yet: the amount is simply transferred there. It used to point
     * at an inventory swap desk, which no longer exists — swaps route through
     * AMM liquidity now, so the leg's real destination is `amm`.
     */
    address public liquidity;
    /// @notice The TesseraAMM this collector funds. When set, the liquidity leg
    ///         funds `ammPoolId` — this is what makes an AMM fee collector.
    address public amm;
    ...
    constructor(address asset_, address agent_, address pool_, address vault_, address liquidity_) {
    ...
    function _allocate() internal {
        ...
        if (toSwap > 0 && amm != address(0)) {
            // Fees go back into the pool, lifting the value of every LP share in
            // it — the app's own position included.
            asset.approve(amm, toSwap);
            try IFeeAmm(amm).fund(ammPoolId, address(asset), toSwap) {} catch {
                asset.approve(amm, 0);
                toSwap = 0;
            }
        } else if (toSwap > 0 && liquidity != address(0)) {
            if (!asset.transfer(liquidity, toSwap)) toSwap = 0;
        } else {
            toSwap = 0;
        }
    ...
    function setSinks(address agent_, address pool_, address vault_, address liquidity_) external onlyOwner {
        agent = agent_;
        pool = pool_;
        vault = vault_;
        liquidity = liquidity_;
        emit SinksSet(agent_, pool_, vault_, liquidity_);
    }

    /// @notice Route the liquidity leg into an AMM pool. Pass `address(0)` to fall
    ///         back to a plain transfer to `liquidity`.
