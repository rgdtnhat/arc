 ...
 * that already names a market is reported and left alone unless `--replace`
 * says otherwise — redeploying would strand every live listing, and the tokens
 * escrowed against them, in the old contract.
 *
...
if (argv.includes("--find")) {
  ...
  console.log(`
  searching ${depth} nonces back from ${nonce} for a market deployed by ${deployerAddress}
`);
  const hits = [];
...
if (existing) {
  ...
  if (live && !REPLACE) {
    console.log(
      "\n  Nothing to do. Redeploying would strand every live listing, and the tokens escrowed\n" +
        "  against them, in the old contract — so it takes --replace to say that is what you meant.\n",
    );
    process.exit(0);
  }
}
...
recordDeployment({ key: "tesseraNftMarket", address, recordUrl: RECORD, record: dep });
